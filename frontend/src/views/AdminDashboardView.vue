...
// when the level to create changes, which scope fields should show?
...
function levelLabel(lvl: string | null): string {
  if (!lvl) return '—'
  const key = lvl.replace('level_', 'level_')
  ...
  <div class="admin-page">
    ...
    <div class="table-wrap">
      <table class="user-table">
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="9" class="empty-cell">{{ i18n.t('loading') || 'Loading…' }}</td>
          </tr>
          <tr v-else-if="rows.length === 0">
            <td colspan="9" class="empty-cell">{{ i18n.t('admin_no_users') }}</td>
          </tr>
          <tr
            v-for="(u, idx) in rows"
            :key="u.id"
          ...
          >
            ...
            <td>{{ u.admin_scope_country || '—' }}</td>
            <td>{{ u.admin_scope_region || '—' }}</td>
            <td>
    ...
    <div v-if="showCreate" class="modal-overlay">
      <div class="modal-card">
        ...
        <div class="modal-actions">
          <button class="btn btn-primary modal-btn" :disabled="creating" @click="submitCreate">
