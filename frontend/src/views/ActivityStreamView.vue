...
async function onStartRecord() {
  ...
  catch (e: any) { alert(i18n.t('recording_upload_failed') + ': ' + (e?.message || '')) }
}
async function onStopRecord() {
  const blob = await recorder.stop()
  if (blob && blob.size > 0) showPreview.value = true
}
...
async function onEndActivity() {
  ...
  if (!confirm('End this activity?')) return

  let blob: Blob | null = null
  if (recorder.isRecording.value) blob = await recorder.stop()
    ...
    <main v-else class="activity-body">
      <section class="left-col">
        ...
        <div class="action-bar">
          ...
          <!-- 单个"录制/停止" -->
          <button
            v-if="isHost" type="button" class="action-btn"
          ...
          <button v-if="isHost" type="button" class="action-btn danger" @click="onEndActivity">
...
<style scoped>
.activity-page { position: fixed; inset: 0; z-index: 9999; display: flex; flex-direction: column; background: #000; color: #fff; overflow: hidden; box-sizing: border-box; }
.activity-top { flex-shrink: 0; background: #0d0d0d; border-bottom: 1px solid #222; }
...
.activity-title-input { flex: 1; min-width: 0; background: #1a1a1a; border: 1px solid #333; border-radius: 8px; color: #fff; padding: 0.4rem 0.7rem; font-size: 0.9rem; font-weight: 600; }
.category-row { padding: 0.35rem 0.7rem 0.5rem; }
.activity-status { display: flex; flex-direction: column; align-items: center; justify-content: center; flex: 1; gap: 1rem; color: #ccc; }
.activity-body { display: grid; grid-template-columns: 3fr 2fr; gap: 0.5rem; padding: 0.5rem; flex: 1 1 auto; min-height: 0; overflow: hidden; }
.is-fullscreen .activity-body { grid-template-columns: 1fr; gap: 0; padding: 0; }
.left-col { display: flex; flex-direction: column; gap: 0.4rem; min-height: 0; min-width: 0; }
...
.right-col { display: grid; grid-template-rows: 3fr 1fr auto; gap: 0.5rem; min-height: 0; min-width: 0; overflow: hidden; }
.activity-stats, .personal-stats { min-height: 0; }
.personal-upload-btn { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.1rem; padding: 0.45rem 0.2rem; background: linear-gradient(135deg, #8b5cf6, #6366f1); border: none; border-radius: 10px; color: #fff; font-size: 1.1rem; font-weight: 700; cursor: pointer; box-shadow: 0 4px 14px rgba(139, 92, 246, 0.4); }
.personal-upload-btn .label { font-size: 0.68rem; color: #fff; font-weight: 500; }
...
@media (max-width: 900px) { .activity-body { grid-template-columns: 1fr; gap: 0.4rem; padding: 0.4rem; overflow-y: auto; } .right-col { grid-template-rows: 300px 200px auto; } }
</style>