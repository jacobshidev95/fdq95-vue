...
function isConsumerRow(u: AdminUserRow): boolean {
...
async function load() {
  ...
  try {
    ...
    actionError.value = extractError(e, i18n.t('admin_load_failed'))
  } finally {
    loading.value = false
  }
}
...
async function freezeUser(u: AdminUserRow) {
  ...
  try {
    ...
    actionError.value = extractError(e, i18n.t('admin_action_failed'))
  }
}
...
/** ★ 显示国家：优先后端 country，再兜底 admin_scope_country */
function displayCountry(u: AdminUserRow): string {
  return (u.country ?? u.admin_scope_country ?? '') || '—'
}

/** ★ 显示区域：优先后端 region，再兜底 admin_scope_region */
function displayRegion(u: AdminUserRow): string {
  return (u.region ?? u.admin_scope_region ?? '') || '—'
}

/** ★ Provider Level 显示：普通用户显示 "Consumer" 徽章 */
function displayLevel(u: AdminUserRow): string {
  if (u.provider_level) return levelLabel(u.provider_level)
  return i18n.t('consumer') || 'Consumer'
}
...
function canAct(u: AdminUserRow): boolean {
  ...
  const myLevel = auth.user.provider_level
  const targetLevel = u.provider_level
  const consumer = isConsumerRow(u)

  if (myLevel === 'level_0') return true

  if (myLevel === 'level_1') {
    if (targetLevel === 'level_0' || targetLevel === 'level_1') return false
    if (targetLevel === 'level_2') {
      return (
        (u.admin_scope_country || u.country) ===
        auth.user.admin_scope_country
      )
    }
    if (consumer) return u.country === auth.user.admin_scope_country
    return false
  }
  ...
  if (myLevel === 'level_2') {
    ...
    if (targetLevel === 'level_2') {
      return (
        (u.admin_scope_country || u.country) ===
          auth.user.admin_scope_country &&
        (u.admin_scope_region || u.region) === auth.user.admin_scope_region
      )
    }
    if (consumer) {
      return (
        u.country === auth.user.admin_scope_country &&
        u.region === auth.user.admin_scope_region
      )
    }
    return false
  }
        ...
        <table class="user-table">
          ...
          <tbody>
            ...
            >
              ...
              <!-- ★ Country / Region：使用兜底函数 -->
              <td>{{ displayCountry(u) }}</td>
              <td>{{ displayRegion(u) }}</td>
              <td>{{ u.role || 'consumer' }}</td>
              <!-- ★ Level：普通用户显示 Consumer 灰徽章 -->
              <td>
                <span
                  v-if="u.provider_level"
                  class="level-badge"
                >{{ displayLevel(u) }}</span>
                <span v-else class="level-badge consumer">
                  {{ displayLevel(u) }}
                </span>
              </td>
              <td>
